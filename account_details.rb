...
class Twitter_Request
  ...
  def initialize(username, count)
    @username = username
    @count = count
    @tweets_words = self.tweet_collect
    @okay_words = self.remove_stop_words
    @words_rank = self.rank_words
  end

  def tweet_collect
    Connection.tweets_request(self.username, self.count)
  ...
  def remove_stop_words
    tweets = self.tweets_words
    stop_words = []
    File.open("stop_words.json", "r").each { |x|
      stop_words << x
    }
...
mubarak = Twitter_Request.new("mubarakadeimam", 5)
